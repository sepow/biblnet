...
class DocumentView(object):
    ...
    def list(self, request, tribe_slug):
        ...
        queryset = self._get_prepared_queryset().filter(tribe=tribe)
        ...
        return render_to_response(self.list_template, context, context_instance=RequestContext(request))

    def get_document(self, id, tribe_slug):
        queryset = self._get_prepared_queryset().filter(tribe__slug=tribe_slug)
    ...
    def detail(self, request, id, tribe_slug):
        tribe = get_object_or_404(Tribe, slug=tribe_slug)
        ...
        return render_to_response(self.template, locals(), context_instance=RequestContext(request))
